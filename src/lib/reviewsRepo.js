...
// The current juror's own review row for an application (or null).
export async function getMyReview(applicationId, userId) {
...
// Upsert the juror's row. `status` is 'draft' or 'finished'. Always writes
// unlocked=false (RLS forbids a juror setting it true).
async function upsertReview({ applicationId, userId, email, scores, status }) {
